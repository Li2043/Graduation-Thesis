"""Stage 8 arm0 single-seed runner (Double DQN + Base Reward V2 baseline, diagnostic).

Near-literal adaptation of `thesis.pilots.stage7c_q1_runner`, substituting the
Stage 8 arm0 constants/guards module. `FormalTrainer` already correctly
threads `active_time_cost_per_step` -- no change needed there.
"""
...
def run_training_job(
    *,
    ...
    if device != "cpu" and strict:
        raise RuntimeError("Stage 8 arm0 device must be cpu unless non-strict")
    algorithm = ALGORITHM
    condition = CONDITION
    if not allow_smoke:
        assert_stage8_arm0_guards(
        ...
        if master_seed in PILOT_SEEDS:
            raise RuntimeError("smoke seed must not be in 65001-65002")
        if (
    ...
    if mode is not DQNTargetMode.DOUBLE:
        raise RuntimeError("Stage 8 arm0 requires Double DQN")
