"""Stage 8 formal qualification gate single-seed runner.

Near-literal adaptation of `thesis.pilots.stage7c_q1_runner` (same 400K-step
scale), substituting the Stage 8 gate constants/guards module. The only
functional difference from Stage 7C-Q1's `build_config()` is
`learning_rate_end=0.0001`, `learning_rate_decay_environment_steps=400_000`
(arm2b's fix, extended to the full gate budget) -- epsilon schedule and
target-update mode are otherwise identical to Stage 7C-Q1/arm0.
"""
...
def build_config(*, max_steps: int = MAX_STEPS) -> FormalConfig:
    ...
    cfg = FormalConfig(
        ...
        dqn=FormalDQNConfig(
            ...
            # target_update_mode left at its default ("hard"); TARGET_UPDATE_MODE
            # is asserted against explicitly in run_training_job for provenance.
        ),
...
def run_training_job(
    *,
    ...
    if device != "cpu" and strict:
        raise RuntimeError("Stage 8 gate device must be cpu unless non-strict")
    algorithm = ALGORITHM
    condition = CONDITION
    if not allow_smoke:
        assert_stage8_gate_guards(
        ...
        if master_seed in PILOT_SEEDS:
            raise RuntimeError("smoke seed must not be in 65021-65040")
        if (
            ...
            or master_seed in FORBIDDEN_STAGE8_PILOT_SEEDS
        ):
    ...
    if mode is not DQNTargetMode.DOUBLE:
        raise RuntimeError("Stage 8 gate requires Double DQN")
