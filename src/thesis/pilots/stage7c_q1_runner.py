"""Stage 7C-Q1 single-seed runner (Double DQN + Base Reward V2 baseline)."""
...
def run_training_job(
    *,
    ...
    if device != "cpu" and strict:
        raise RuntimeError("Stage 7C-Q1 formal device must be cpu unless non-strict")
    algorithm = ALGORITHM
    condition = CONDITION
    if not allow_smoke:
        assert_stage7c_q1_guards(
        ...
        if master_seed in PILOT_SEEDS:
            raise RuntimeError("smoke seed must not be in 64001-64020")
        if master_seed in range(61001, 61011) or master_seed in range(62001, 62021) or master_seed in range(63001, 63021):
    ...
    if mode is not DQNTargetMode.DOUBLE:
        raise RuntimeError("Stage 7C-Q1 requires Double DQN")
    ...
    trainer = FormalTrainer(
        ...
        active_time_cost_per_step=ACTIVE_TIME_COST_PER_STEP if not allow_smoke else ACTIVE_TIME_COST_PER_STEP,
    )
    ...
    def save_step(step: int) -> dict[str, Any]:
        ...
        meta = {
            ...
            "storage_location": "experiment_machine_local",
        }
