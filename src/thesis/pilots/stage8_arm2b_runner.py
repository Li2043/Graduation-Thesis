"""Stage 8 arm2b single-seed runner (Double DQN + Base Reward V2 baseline,
linear learning-rate decay 0.0005 -> 0.0001 over the full 100K-step budget).

Near-literal adaptation of `thesis.pilots.stage8_arm1_runner`, substituting
the Stage 8 arm2b constants/guards module. The only functional difference
from arm0's `build_config()` is `learning_rate_end=0.0001` +
`learning_rate_decay_environment_steps=100_000` instead of arm0's constant
0.0005 -- every other training hyperparameter (including the epsilon
schedule and target-sync mode, unlike arm1/arm2a), the reward, and the
algorithm are identical to arm0.
"""
...
from thesis.pilots.stage8_arm2b_config import (
...
def run_training_job(
    *,
    ...
    if device != "cpu" and strict:
        raise RuntimeError("Stage 8 arm2b device must be cpu unless non-strict")
    algorithm = ALGORITHM
    condition = CONDITION
    if not allow_smoke:
        assert_stage8_arm2b_guards(
        ...
        if master_seed in PILOT_SEEDS:
            raise RuntimeError("smoke seed must not be in 65007-65014")
        if (
            ...
            or master_seed in FORBIDDEN_STAGE8_ARM1_SEEDS
            or master_seed in FORBIDDEN_STAGE8_ARM2A_SEEDS
        ):
    ...
    if mode is not DQNTargetMode.DOUBLE:
        raise RuntimeError("Stage 8 arm2b requires Double DQN")
