"""Stage 8 arm2a single-seed runner (Double DQN + Base Reward V2 baseline,
soft target-network update replacing arm0's hard periodic copy).

Near-literal adaptation of `thesis.pilots.stage8_arm1_runner`, substituting
the Stage 8 arm2a constants/guards module. The only functional difference
from arm0's `build_config()` is `target_update_mode="soft"` +
`target_soft_tau=0.005` instead of the default hard copy every 250 updates
-- every other training hyperparameter (including the epsilon schedule,
unlike arm1), the reward, and the algorithm are identical to arm0.
"""
...
from thesis.pilots.stage8_arm2a_config import (
...
def run_training_job(
    *,
    ...
    if device != "cpu" and strict:
        raise RuntimeError("Stage 8 arm2a device must be cpu unless non-strict")
    algorithm = ALGORITHM
    condition = CONDITION
    if not allow_smoke:
        assert_stage8_arm2a_guards(
        ...
        if master_seed in PILOT_SEEDS:
            raise RuntimeError("smoke seed must not be in 65005-65006")
        if (
    ...
    if mode is not DQNTargetMode.DOUBLE:
        raise RuntimeError("Stage 8 arm2a requires Double DQN")
