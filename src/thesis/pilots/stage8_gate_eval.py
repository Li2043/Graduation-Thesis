"""Stage 8 gate greedy evaluation with rich per-step logging.

Adapted from `thesis.pilots.stage8_arm0_eval` (same V2-reward-correct
env-construction path spliced with the rich per-step diagnostic schema),
substituting `thesis.pilots.stage8_gate_config` for
`thesis.pilots.stage8_arm0_config` -- the critical difference: arm0's
`n_scenario_blocks`/`episodes_per_seed_checkpoint` only ever handles
checkpoints <= 175,000 (arm0 never trained past 100,000 steps) and RAISES
for anything beyond. The gate trains to 400,000 steps and needs checkpoints
up to 400,000 evaluated, which requires the "late" 32-scenario-block plan
(checkpoint > 175,000) -- this module's config functions support that
(mirroring `thesis.pilots.stage7c_q1_config`, which they are copied from).

Only used for the `RICH_LOG_CHECKPOINTS` subset (0, 350K, 375K, 400K) --
see `stage8_gate_config.py` docstring. The other 13 checkpoints use
`thesis.pilots.stage7c_q1_eval.evaluate_checkpoint_stage7c` directly
(lightweight, episode-level only, no per-step Q-value logging), which
already correctly handles both the early (8-block) and late (32-block)
plans since it imports its block-count logic from `stage7c_q1_config`
(numerically identical to this module's for every checkpoint the gate uses).
"""
...
def _block_for_template(
    ...
    """Copied inline from stage7c_q1_eval.py (private helper, not imported).
    `template_index % 8` is correct for BOTH the 8-block early plan and the
    32-block late plan: `eval_plan_for_checkpoint` always derives
    `template_validation_index = scenario_block % 8`, so scenario_block
    0..31 cycles through the same 8 physical templates 4x, each with a
    distinct SHA-256-derived eval_seed."""
...
def run_greedy_episode_diagnostic(
    ...
    *,
    ...
    Identical logic to stage8_arm0_eval.run_greedy_episode_diagnostic -- not
    re-derived, just re-hosted in this module so the gate's config imports
    are self-contained.
    """
...
def evaluate_checkpoint_stage8_gate(
    bundle: FinalLockBundle,
    learners: dict[str, Any],
    *,
    master_seed: int,
    ...
    """Greedy eval across the early (8-block) or late (32-block) plan
    depending on checkpoint_step; must not mutate learners."""
    before = {
...
__all__ = [
    "evaluate_checkpoint_stage8_gate",
