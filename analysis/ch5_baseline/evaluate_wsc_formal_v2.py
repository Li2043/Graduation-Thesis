"""WSC v2 (post-fix) checkpoint-Q ensemble evaluation. Identical episode
logic to evaluate_wsc_formal.py (unchanged), pointed at the CORRECTED
wsc_formal_runs_v2 checkpoints instead of the invalid pre-fix
wsc_formal_runs ones.

IMPORTANT CAVEAT printed and written with every output: any run of this
script against fewer than all 12 formal seeds is an INTERIM/PARTIAL,
INFORMAL read, not a formal result. This project's own frozen-protocol
discipline treats early looks at accumulating seeds as unreliable given
known seed-level heterogeneity (see the Original experiment's own
seed-level deltas, e.g. Mean-Baseline Delta_Umin ranging from -0.678 to
+0.347 across seeds). This script and its output must NOT be used for any
thesis table/figure/formal conclusion.
"""
...
OUT_DIR = Path(__file__).resolve().parent / "outputs" / "wsc_interim_v2"
...
def ckpt_dir_for(cond: str, seed: int) -> Path:
    return WSC_ROOT / f"{cond}_wsc_{seed}" / f"seed_{seed}_Formal_{cond}_WSC_v2"
...
def run_one(cond: str, seed: int, bank_name: str = "H1") -> list[dict]:
    ...
    agents = load_ensemble_agents(
        ...
        expected_stage_by_step=dict.fromkeys(WINDOW, f"Formal_{cond}_WSC_v2"),
...
def main() -> int:
    ...
    print("=" * 78)
    print("INTERIM WSC v2 (POST-FIX) EVALUATION -- CAVEAT: covers only the seeds listed")
    print("below, NOT the full 12-seed formal sample. NOT a formal result -- for")
    print("thesis-direction purposes only, per explicit user request.")
    ...
    for cond in CONDITIONS:
        for seed in args.seeds:
            if not all_checkpoints_exist(cond, seed):
                ...
                continue
            print(f"[evaluate_wsc_formal_v2] {cond}_wsc_{seed} on H1 ...", flush=True)
    ...
    out_csv = OUT_DIR / f"wsc_interim_v2_evaluation{suffix}.csv"
