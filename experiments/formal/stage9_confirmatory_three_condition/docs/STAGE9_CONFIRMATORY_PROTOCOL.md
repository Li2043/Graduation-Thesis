...
| coordination resolution (q, certified choice states) | ≤ 0.05 (5 points) absolute decrease vs. baseline | Kept at its original value — see the pre-registered power caveat immediately below. `MES_SWAP_ELIGIBILITY` (§7.1, RQ2) separately corrected from 0.20 to 0.25 using a freshly-computed per-seed SD (0.269 vs the coarser 0.311 estimate used when §7.1 was first drafted) — that correction is legitimate (a detection-floor MES for a difference test is properly derived from n and variance) and is unrelated to the margin issue below (a non-inferiority margin must instead come from a substantive judgement of tolerable loss, not from achievable precision). |

**Pre-registered power caveat for q, written before mean_pbrs/min_pbrs training finishes:** self-testing `run_stage9_decision.py` (baseline compared against a copy of itself, to exercise the statistical code before real comparison data existed) surfaced that the 0.05 margin above is not cleanly resolvable at n=20 — q's real per-seed SD (certified states, pooled over the 3 gate checkpoints) is 0.163, giving a one-sided CI half-width of ≈0.101, roughly double the margin. An earlier draft of this document widened the margin to 0.12 to make the test "resolvable" — reverted the same day: choosing a margin based on what a fixed sample size can detect, rather than a substantive judgement of tolerable loss, inverts the correct logic of non-inferiority testing (a documented anti-pattern; cf. ICH E9 guidance against power-driven margin selection). The margin stays at 0.05. Instead, `non_inferiority_test()` (`stage9_analysis.py`) now returns one of **three** outcomes — `non_inferior` / `inferior` / `inconclusive` — rather than a binary true/false: `inconclusive` means the point estimate does not show a loss beyond the margin, but the confidence interval is too wide to rule one out. **The q-based RQ3 claim is expected, in advance, to most likely land `inconclusive` at n=20** — this must be reported as "underpowered to resolve this margin," never presented as evidence of either non-inferiority or harm.
