...
| coordination resolution (q, certified choice states) | ≤ 0.12 (12 points) absolute decrease vs. baseline | **CORRECTED 2026-08-03**, originally frozen at 0.05 the same day. Building `run_stage9_decision.py` and self-testing it (baseline compared against a copy of itself) surfaced that 0.05 is not achievable at n=20: q's real per-seed SD (certified states, pooled over the 3 gate checkpoints) is 0.163, giving a one-sided CI half-width of ≈0.101 — roughly double the original margin, meaning the test would return `non_inferior: false` close to by construction even for a genuinely zero difference. Widened to 0.12, just above the achievable ≈0.101 precision. `MES_SWAP_ELIGIBILITY` (§7.1, RQ2) similarly corrected from 0.20 to 0.25 using the same freshly-computed per-seed SD (0.269 vs the coarser 0.311 estimate used when §7.1 was first drafted). |
