#!/usr/bin/env python3
"""Parallel launcher for Stage 8 arm0 (2 Double-DQN baseline diagnostic seeds)."""
...
JOB_SCRIPT = SCRIPT.parent / "run_stage8_arm0_training.py"
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        "--protocol",
        type=Path,
        default=PILOT_ROOT / "configs" / "stage8_arm0_protocol.yaml",
    )
    ...
    if len(seeds) > 2:
        print(f"ABORT: {len(seeds)} seeds exceeds frozen arm0 seed count (2)", flush=True)
        return 2

    write_sem = threading.Semaphore(int(args.checkpoint_write_slots))
    lock_path = PILOT_ROOT / "logs" / "launch_stage8_arm0.lock"
