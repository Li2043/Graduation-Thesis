#!/usr/bin/env python3
"""Stage 8 arm0 evaluation driver.

Loops seeds x checkpoints, loads each checkpoint's saved learner state, and
calls evaluate_checkpoint_stage8_arm0(..., collect_trajectories=True). Writes
a flat evaluation_episodes.csv (same schema as Stage 7C-Q1's, for direct
comparability) plus per-seed-per-checkpoint trajectory CSVs.

Nothing like this exists in the reference tree even for Stage 7C-Q1 (its
evaluation_episodes.csv was produced by an ad-hoc/uncommitted driver) -- this
is a new script, not an adaptation of an existing one.
"""
...
from thesis.pilots.stage8_arm0_config import (  # noqa: E402
    CHECKPOINT_STEPS,
    PILOT_SEEDS,
    PROTOCOL_TAG,
)
...
def _append_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    """Copied from stage7a1_runner.py::_append_csv (generic, unions fieldnames)."""
    if not rows:
        return
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        "--protocol",
        type=Path,
        default=Path(__file__).resolve().parents[1] / "configs" / "stage8_arm0_protocol.yaml",
    )
    p.add_argument("--seeds", default="65001,65002")
    ...
    for s in seeds:
        if s not in PILOT_SEEDS:
            raise RuntimeError(f"seed {s} not in frozen arm0 seed block {PILOT_SEEDS}")
    for c in checkpoints:
        if c not in CHECKPOINT_STEPS:
            raise RuntimeError(f"checkpoint {c} not in frozen arm0 checkpoint list {CHECKPOINT_STEPS}")
