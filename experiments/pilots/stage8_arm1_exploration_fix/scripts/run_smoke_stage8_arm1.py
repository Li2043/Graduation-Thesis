#!/usr/bin/env python3
"""Local smoke test for Stage 8 arm1 (temporary seed, short budget, external ckpt root).

Adapted from run_smoke_stage8_arm0.py. Validates the single-variable change
(epsilon_decay_environment_steps=75000 instead of arm0's 50000) actually
threads through to the saved checkpoint metadata, then reuses arm0's
run_greedy_episode_diagnostic (eval is greedy, epsilon-independent, so it is
correctly shared across arms) to confirm the rich per-step trajectory fields
...
def main() -> int:
    ...
    from thesis.pilots.stage8_arm1_runner import run_training_job
    from thesis.pilots.stage8_arm1_config import EPSILON_DECAY_STEPS
    ...
    protocol = PILOT_ROOT / "configs" / "stage8_arm1_protocol.yaml"
    smoke_seed = 65998  # outside frozen (65003-65004) / arm0 (65001-65002) / historical blocks
    max_steps = 2000  # > replay_warmup_per_controller (512): exercises real updates

    with tempfile.TemporaryDirectory(prefix="stage8_arm1_smoke_") as td:
        td_path = Path(td)
        ...
        assert int(meta0["epsilon_decay_environment_steps"]) == 75_000 == EPSILON_DECAY_STEPS, meta0
        ...
        # so it does not depend on the training-time epsilon schedule).
        ...
        from thesis.pilots.stage8_arm1_config import PROTOCOL_TAG
        ...
        report = {
            ...
            "seed": smoke_seed,
            "max_steps": max_steps,
            "epsilon_decay_environment_steps_confirmed": 75_000,
