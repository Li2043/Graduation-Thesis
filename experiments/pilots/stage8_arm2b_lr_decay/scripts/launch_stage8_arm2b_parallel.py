#!/usr/bin/env python3
"""Parallel launcher for Stage 8 arm2b (2 Double-DQN baseline diagnostic seeds,
learning-rate decay fix: linear 0.0005 -> 0.0001 over 100K steps)."""
...
JOB_SCRIPT = SCRIPT.parent / "run_stage8_arm2b_training.py"

sys.path.insert(0, str(REPO_ROOT / "src"))
from thesis.pilots.stage8_arm2b_config import PILOT_SEEDS  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        "--protocol",
        type=Path,
        default=PILOT_ROOT / "configs" / "stage8_arm2b_protocol.yaml",
    )
    ...
    for s in seeds:
        if s not in PILOT_SEEDS:
            print(f"ABORT: seed {s} not in frozen arm2b seed block {PILOT_SEEDS}", flush=True)
            return 2

    write_sem = threading.Semaphore(int(args.checkpoint_write_slots))
    lock_path = PILOT_ROOT / "logs" / "launch_stage8_arm2b.lock"
