#!/usr/bin/env python3
"""Stage 8 arm2b evaluation driver.

Loops seeds x checkpoints, loads each checkpoint's saved learner state, and
calls evaluate_checkpoint_stage8_arm0(..., protocol_tag=stage8_arm2b's tag,
collect_trajectories=True) -- reused unchanged from arm0 since evaluation is
always greedy (epsilon=0) and therefore independent of arm2b's single
training-time change (target_update_mode). Writes a flat
...
from thesis.pilots.stage8_arm2b_config import (  # noqa: E402
    CHECKPOINT_STEPS,
    PILOT_SEEDS,
    PROTOCOL_TAG,
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        "--protocol",
        type=Path,
        default=Path(__file__).resolve().parents[1] / "configs" / "stage8_arm2b_protocol.yaml",
    )
    p.add_argument("--seeds", default="65009,65010,65011,65012,65013,65014")
    ...
    for s in seeds:
        if s not in PILOT_SEEDS:
            raise RuntimeError(f"seed {s} not in frozen arm2b seed block {PILOT_SEEDS}")
    for c in checkpoints:
        if c not in CHECKPOINT_STEPS:
            raise RuntimeError(f"checkpoint {c} not in frozen arm2b checkpoint list {CHECKPOINT_STEPS}")
