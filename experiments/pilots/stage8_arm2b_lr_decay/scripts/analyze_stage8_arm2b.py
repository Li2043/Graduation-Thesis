#!/usr/bin/env python3
"""Stage 8 arm2b analysis: descriptive checkpoint table + frozen_stall vs
moving_with_switches mode split + training-stability comparison against
arm0 and arm1.

Structurally a copy of analyze_stage8_arm2a.py, with the description text
adapted for arm2b's mechanism (learning-rate decay, not soft target update):
  (a) per-seed success_rate volatility (max - min success_rate across the
      ...
      arm2b's actual target (linear LR decay is meant to reduce
      checkpoint-to-checkpoint swings caused by overly large late-training
      gradient steps), not just the 100K-checkpoint frozen_stall count that
      arm1 targeted;
  (b) the comparison pulls in arm1's results too (not just arm0's), since
      arm2b is a sibling single-variable arm to arm1 and arm2a, not a
      replacement for either -- STAGE8_PLAN_DRAFT.md SS4 frames all three
      (arm1, arm2a, arm2b) as separate tests against the same arm0 baseline.
"""
...
def _volatility(ckpt: pd.DataFrame) -> pd.DataFrame:
    ...
    This is arm2b/arm2b's actual target mechanism (seed-level training
...
def main() -> int:
    ...
    frozen_comparison, volatility_comparison, notes = _cross_arm_comparison(ckpt_summary, diag, "arm2b")
    if not frozen_comparison.empty:
        frozen_comparison.to_csv(OUT_ROOT / "comparison_frozen_stall_vs_arm0_arm1.csv", index=False)
        print("\n=== frozen_stall comparison (arm0 / arm1 / arm2b) ===")
    ...
    if not volatility_comparison.empty:
        ...
        print("\n=== success_rate volatility comparison (arm0 / arm1 / arm2b) ===")
        print(volatility_comparison.to_string(index=False))
    for n in notes:
        print(n)

    note_lines = [
        "# Stage 8 arm2b — Learning-Rate Decay Fix, Diagnostic Note",
        "",
        "**Status: diagnostic pilot only. No PASS/FAIL claim.**",
        "",
        "Single variable changed vs arm0: linear learning-rate decay",
        "`learning_rate_end=0.0001`, `learning_rate_decay_environment_steps=100000`",
        "(epsilon schedule, target-update mode, reward, and algorithm unchanged).",
        "",
    ...
    if notes:
        ...
        note_lines.append("")
    note_lines += [
        "Interpretation guide: arm2b targets seed-level training instability",
    ...
    (OUT_ROOT / "STAGE8_ARM2B_NOTE.md").write_text("\n".join(note_lines) + "\n", encoding="utf-8")
    print(f"\nwrote {OUT_ROOT / 'STAGE8_ARM2B_NOTE.md'}")
    return 0
